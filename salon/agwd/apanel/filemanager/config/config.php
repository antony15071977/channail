...
//----------------------------------------AGWD
$ag_index = 1;
if (!isset($ag_this_p)) {$ag_this_p = '../../';}

include($ag_this_p.'inc/host.php');

include($ag_this_p.'inc/db_conf.php');
include($ag_this_p.$ag_data_dir. '/'. $ag_config);


if (!empty($ag_cfg_time_zone)) { $ag_cfg_time_zone = str_replace('_', '/', $ag_cfg_time_zone); date_default_timezone_set($ag_cfg_time_zone); } 
include($ag_this_p.$ag_cfg_lng);
include ($ag_this_p.'inc/mobile_detect/mobile_detect.php'); 
include($ag_this_p.'inc/functions.php');
$ag_re_data_dir = $ag_data_dir;
$ag_data_dir_fm = $ag_this_p.$ag_data_dir;
$ag_data_dir = $ag_data_dir_fm;
include($ag_this_p.'inc/auth.php');
$ag_data_dir = $ag_re_data_dir;

$srv_current_path_arr = explode('/',$srv_current_path);
array_pop($srv_current_path_arr);
$ag_fm_dir = '';
$ag_ap_dir = '';
if (isset($srv_current_path_arr[(sizeof($srv_current_path_arr) - 1)])) {$ag_fm_dir = $srv_current_path_arr[(sizeof($srv_current_path_arr) - 1)];}
if (isset($srv_current_path_arr[(sizeof($srv_current_path_arr) - 2)])) {$ag_ap_dir = $srv_current_path_arr[(sizeof($srv_current_path_arr) - 2)];}

$ag_dir = $_SERVER['PHP_SELF'];
$ag_dir = str_replace($ag_ap_dir.'/'.$ag_fm_dir.'/', '', $ag_dir);
$ag_dir = str_replace(basename($ag_dir), '', $ag_dir);
...
//===============================================================================
//access
if ($ag_user_access == 'founder' || $ag_user_access == 1) { // admin access

    $ag_delete_files                            = true;
	$ag_create_folders                          = true;
	$ag_delete_folders                          = true;
	$ag_upload_files                            = true;
	$ag_rename_files                            = true;
	$ag_rename_folders                          = true;
	$ag_duplicate_files                         = true;
	$ag_copy_cut_files                          = true; // for copy/cut files
	$ag_copy_cut_dirs                           = true; // for copy/cut directories
	$ag_chmod_files                             = true; // change file permissions
	$ag_chmod_dirs                              = true; // change folder permissions
	$ag_preview_text_files                      = true; // eg.: txt, log etc.
	$ag_edit_text_files                         = true; // eg.: txt, log etc.
	$ag_create_text_files                       = true; // only create files with exts. defined in $editable_text_file_exts
	$js_v = 'js';
	
} else { // user access
	
	$ag_delete_files                            = false;
	$ag_create_folders                          = true;
	$ag_delete_folders                          = false;
	$ag_upload_files                            = true;
	$ag_rename_files                            = false;
	$ag_rename_folders                          = false;
	$ag_duplicate_files                         = true;
	$ag_copy_cut_files                          = false; // for copy/cut files
	$ag_copy_cut_dirs                           = false; // for copy/cut directories
	$ag_chmod_files                             = false; // change file permissions
	$ag_chmod_dirs                              = false; // change folder permissions
	$ag_preview_text_files                      = false; // eg.: txt, log etc.
	$ag_edit_text_files                         = false; // eg.: txt, log etc.
	$ag_create_text_files                       = false; // only create files with exts. defined in $editable_text_file_exts
...
$config = array(
	...
	*/
	...
	|
	*/
	'upload_dir' => $ag_dir.$ag_data_dir.'/'.$ag_upload_name.'/',
	/*
	|--------------------------------------------------------------------------
	...
	|
	*/
	'current_path' => '../../'.$ag_data_dir.'/'.$ag_upload_name.'/',
	...
	|
	*/
	'thumbs_base_path' => '../../'.$ag_data_dir.'/thumbs/',
	...
	*/
	...
	//******************
	'delete_files'                            => $ag_delete_files,
	'create_folders'                          => $ag_create_folders,
	'delete_folders'                          => $ag_delete_folders,
	'upload_files'                            => $ag_upload_files,
	'rename_files'                            => $ag_rename_files,
	'rename_folders'                          => $ag_rename_folders,
	'duplicate_files'                         => $ag_duplicate_files,
	'copy_cut_files'                          => $ag_copy_cut_files, // for copy/cut files
	'copy_cut_dirs'                           => $ag_copy_cut_dirs, // for copy/cut directories
	'chmod_files'                             => $ag_chmod_files, // change file permissions
	'chmod_dirs'                              => $ag_chmod_dirs, // change folder permissions
	'preview_text_files'                      => $ag_preview_text_files, // eg.: txt, log etc.
	'edit_text_files'                         => $ag_edit_text_files, // eg.: txt, log etc.
	'create_text_files'                       => $ag_create_text_files, // only create files with exts. defined in $editable_text_file_exts
	...
	'ext_file'                                => array( 'doc', 'docx', 'rtf', 'pdf', 'xls', 'xlsx', 'txt', 'csv', 'html', 'xhtml', 'psd', 'sql', 'log', 'fla', 'xml', 'ade', 'adp', 'mdb', 'accdb', 'ppt', 'pptx', 'odt', 'ots', 'ott', 'odb', 'odg', 'otp', 'otg', 'odf', 'ods', 'odp', 'css', 'ai', 'kmz','dwg', 'dxf', 'hpgl', 'plt', 'spl', 'step', 'stp', 'iges', 'igs', 'sat', 'cgm', $js_v), //Files
	...
	*******************/
	...
	'fixed_path_from_filemanager'             => array('../../'.$ag_data_dir.'/'.$ag_mob_images.'/'), //fixed path of the image folder from the current position on upload folder
	...
	'fixed_image_creation_width'              => array( 800, 1200 ), //width of image (you can leave empty if you set height)
